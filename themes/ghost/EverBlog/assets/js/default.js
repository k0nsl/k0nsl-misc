    ...
    if (getCookie("themeStyle") == "") {
        if (default_theme == "default") {
            ...
            if (h >= 7 && h <= 20) {
                ...
                setCookie("themeStyle", "natsoc");
            }
        } else if (default_theme == "natsoc") {
            setCookie("themeStyle", "natsoc");
        } else {
            setCookie("themeStyle", "light");
        }
...
function loadBlogByURL(url, isSetReplaceState, isChangeScreen) {
    $.ajax({
        ...
        error: function () {
            alert("The page failed to load for some inexplicable reason. Try again!");
        }
    });
}
...
function loadBlogListNextPage() {
    ...
    $('<div></div>').load(url, function (responseText, textStatus, XMLHttpRequest) {
        if (textStatus == "success") {
            ...
            scope.html('The page failed to load for some inexplicable reason. Try again!');
        }
        loading = false;
    });
}
...
function bindScroll4Ever() {
    $("#blog-list .scroll").scroll(function () {
        ...
        //Scroll bar height + +100 rolling distance> element in the actual height
...
function changeTheme(theme) {
    if (theme == "light") {
        ...
        $("#style-changer #natsoc").css('display', 'inline-block');
        loadCss(light_theme);
        setCookie("themeStyle", "light");
    } else {
        $("#style-changer #light").css('display', 'inline-block');
        $("#style-changer #natsoc").css('display', 'none');
        loadCss(natsoc_theme);
        setCookie("themeStyle", "natsoc");
    }
}

// let's say who I am
console.log("This blog is operated by k0nsl (i.am@k0nsl.org)");
console.log("https://k0nsl.org");